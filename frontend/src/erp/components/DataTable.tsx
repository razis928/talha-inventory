...
export default function DataTable<T>({
  columns, data, page, pageSize, onPageChange, onEdit, onDelete, onRowClick, showActions = true, keyExtractor,
}: DataTableProps<T>) {
    ...
    <div className="overflow-hidden">
      <div className="overflow-x-auto">
        <table className="erp-classic-table min-w-[640px]">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan={columns.length + (showActions ? 1 : 0)} className="erp-muted py-8 text-center">
                  No records found.
                </td>
              </tr>
            ) : (
              rows.map((row) => (
                <tr
                  ...
                  className={onRowClick ? 'cursor-pointer hover:bg-[var(--color-erp-table-head)]/40' : undefined}
                  ...
                  {showActions && (
                    <td className="text-right" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-1">
                        {onEdit && (
                          <button type="button" onClick={() => onEdit(row)} className="erp-btn-ghost p-1" title="Edit">
                            <Pencil size={14} />
                          </button>
                        )}
                        {onDelete && (
                          <button type="button" onClick={() => onDelete(row)} className="erp-btn-ghost p-1 text-red-700 dark:text-red-400" title="Delete">
                            <Trash2 size={14} />
                          </button>
                        )}
                      </div>
      ...
      <div className="erp-card-2 flex flex-col gap-2 border-t border-[var(--color-erp-border)] px-3 py-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="erp-text text-xs font-bold">Page {page}/{totalPages}</span>
