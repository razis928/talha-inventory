...
export interface AccountPayableSubsidaryLedger {
  id: string;
  company_name: string;
  address: string;
 ...
 */
export const generatePayableSubsidaryLedgerReport = async (
  payables: Vendor[],
): Promise<AccountPayableSubsidaryLedger[]> => {
  ...
  // Group vendor invoices and then enrich with client invoices
  const vendorInvoices = groupCompanyInvoices(
    payableTransactions,
    'company_name',
  );

  // Calculate and update balances in a single pass
  vendorInvoices.forEach((vendor) => {
...
export const groupCompanyInvoices = (list: List, key: AllowedKeys) => {
  const grouped = list.reduce<Map<string, AccountPayableSubsidaryLedger>>(
    (acc, item) => {
