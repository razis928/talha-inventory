...
import {
  clientFormFields,
  ClientFormSchema,
} from '@/app/dashboard/clients/_components/client-form-config';
import LineItemsTable from '@/app/dashboard/clients/_components/line-items-table';
import ClientDetailsModal from '@/app/dashboard/clients/_components/view-client-modal';
...
interface Props {
  type: 'add' | 'update';
  initialState?: z.infer<typeof ClientFormSchema>;
}
const ClientForm: FC<Props> = ({ type, initialState }: Props) => {
  ...
  const onSubmit = useCallback(
    async (data: z.infer<typeof ClientFormSchema>) => {
      setLoading(true);
      ...
      const vendorDetails: CSVTransaction = {
        ...
        address_1: vendorForm?.address_1 || '',
      ...
      const res = await fetch('/api/s3/transactions', {
        ...
        body: JSON.stringify({
          transactions: [clientDetails, vendorDetails],
          key,
          updateRecord: type === 'update' ? true : false,
        }),
      });
      ...
      // if (id) {
      //   const updatedClient = await updateClient(id as string, {
      //     ...rest,
      ...
      // } else {
      //   const [createdClient, _createdVendor] = await Promise.all([
      //     addNewClient({
      //       ...rest,
      //       user_id: userDetails.id,
      ...
      //       company_tagging_id: data.company_tagging_id,
      //       transaction_tagging_id: data.transaction_tagging_id,
      //     }),
      //   ]);
    ...
    [
      form,
      type,
      vendorForm?.address_1,
      vendorForm?.company_name,
      vendorForm?.phone,
    ],
  );

  /**
   * Handles the upload of a file by sending it to the '/api/textract' endpoint
   * for extraction. The function extracts data from the uploaded file and updates
   * the form with the extracted values. It performs the following steps:
   *
   * 1. Creates a FormData object and appends the file to it.
   * 2. Sends the FormData to the '/api/textract' endpoint via a POST request.
   * 3. If the response is not ok, it throws an error with the response's error message.
   * 4. Parses the JSON response to get the extracted data.
   * 5. Extracts `clientForm` and `vendorForm` values from the response data.
   * 6. If `clientForm.tax` is not available, calculates the tax using the city name
   *    and sets it in the form and `vendorForm`.
   * 7. Fetches the tagging ID based on the client's company name.
   * 8. Updates the vendor form with extracted values and resets the form with the
   *    newly extracted data and tagging ID.
   *
   * @param {File} file - The file to be uploaded and processed.
   */
  const fileUploadHandler = async (file: File) => {
    try {
      setIsExtracting(true);
      const { clientForm, vendorForm } = await extractFormFromDocument(file);
      setVendorForm(vendorForm as VendorForm);
      form.reset({
        ...clientForm,
  ...
  const onFieldBlurHandler = useCallback(
    async (companyName: string) => {
      ...
      try {
        const company_tagging_id = await getCompanyTaggingCode(companyName);
        form.setValue('company_tagging_id', company_tagging_id || '');
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='mb-7 flex flex-row items-center gap-5'>
                    ...
                    <FormControl
                      className={`relative flex w-full flex-col items-center ${item.type === 'file' ? 'h-[120px] rounded-lg border border-gray-500 border-opacity-45 md:w-1/4' : 'md:w-[56%]'}`}
                    >
                      {item.type === 'file' ? (
                        <div
                        ...
                        >
                          {isExtracting && (
                            <Loader2 className='absolute h-20 w-20 animate-spin text-gray-500' />
                          )}
                          <Input
                            id={item.id}
                            type={item.type}
